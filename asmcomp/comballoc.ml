...
let rec combine i allocstate =
  ...
  | Iop(Ialloc sz) ->
      begin match allocstate with
        No_alloc ->
          ...
          (instr_cons_debug (Iop(Ialloc newsz)) i.arg i.res i.dbg newnext, 0)
      | Pending_alloc(reg, ofs) ->
          if ofs + sz < Config.max_young_wosize * Arch.size_addr then begin
            ...
            (instr_cons_debug (Iop(Iintop_imm(Iadd, ofs))) [| reg |] i.res i.dbg newnext,
             newsz)
          end else begin
            let (newnext, newsz) =
              combine i.next (Pending_alloc(i.res.(0), sz)) in
            (instr_cons_debug (Iop(Ialloc newsz)) i.arg i.res i.dbg newnext, ofs)
          end
      end
  ...
  | Iifthenelse(test, ifso, ifnot) ->
      ...
      let newnext = combine_restart i.next in
      (instr_cons_debug (Iifthenelse(test, newifso, newifnot)) i.arg i.res i.dbg newnext,
       allocated_size allocstate)
  | Iswitch(table, cases) ->
  ...
  | Icatch(io, body, handler) ->
      ...
      let newnext = combine_restart i.next in
      (instr_cons_debug (Icatch(io, newbody, newhandler)) i.arg i.res i.dbg newnext, sz)
  | Itrywith(body, handler) ->
      let (newbody, sz) = combine body allocstate in
      let newhandler = combine_restart handler in
      let newnext = combine_restart i.next in
      (instr_cons_debug (Itrywith(newbody, newhandler)) i.arg i.res i.dbg newnext, sz)
