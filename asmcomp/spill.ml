...
let add_reloads regset i =
  Reg.Set.fold
    (fun r i -> instr_cons_debug (Iop Ireload) [|spill_reg r|] [|r|] i.dbg i)
    regset i
...
let rec reload i before =
  ...
  | Iloop(body) ->
      ...
      (instr_cons_debug (Iloop(!final_body)) i.arg i.res i.dbg new_next,
       finally)
  | Icatch(nfail, body, handler) ->
      ...
      (instr_cons_debug (Icatch(nfail, new_body, new_handler)) i.arg i.res i.dbg new_next,
       finally)
  | Iexit nfail ->
  ...
  | Itrywith(body, handler) ->
      ...
      let (new_next, finally) =
        reload i.next (Reg.Set.union after_body after_handler) in
      (instr_cons_debug (Itrywith(new_body, new_handler)) i.arg i.res i.dbg new_next,
       finally)
  | Iraise ->
...
let add_spills regset i =
  Reg.Set.fold
    (fun r i -> instr_cons_debug (Iop Ispill) [|r|] [|spill_reg r|] i.dbg i)
    regset i
...
let rec spill i finally =
  ...
  | Iswitch(index, cases) ->
      ...
      (instr_cons_debug (Iswitch(index, new_cases)) i.arg i.res i.dbg new_next,
       !before)
  | Iloop(body) ->
